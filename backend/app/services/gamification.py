...
def record_quiz_rewards(database: Session, user_id: str, attempt: QuizAttempt, correct_count: int, question_count: int) -> None:
    ...
    database.add(XPTransaction(user_id=user_id, amount=xp_amount, source_type="quiz_attempt", source_id=attempt.id))
    today = datetime.now(UTC).date()
    streak = database.get(Streak, user_id) or Streak(user_id=user_id)
    ...
    if goal is None:
        goal = DailyGoal(user_id=user_id, goal_date=today)
        database.add(goal)
    goal.completed_questions += question_count
    _award_badges(database, user_id, attempt, streak)


def _award_badges(database: Session, user_id: str, attempt: QuizAttempt, streak: Streak) -> None:
    ...
    if attempt.score_percent == 100: eligible.add("perfect_quiz")
...
def progression_summary(database: Session, user_id: str) -> dict[str, int]:
    ...
    return {"total_xp": total_xp, "level": level_for_xp(total_xp), "xp_to_next_level": 100 - total_xp % 100, "streak_days": streak.current_days if streak else 0}
