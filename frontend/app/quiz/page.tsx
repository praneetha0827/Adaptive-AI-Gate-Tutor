...
function QuizPageContent() {
  ...
  if (result) return <main className="form-page"><section className="quiz-celebration"><div className="celebration-dots" aria-hidden="true"><i /><i /><i /><i /><i /></div><p className="eyebrow">Practice complete</p><h1>{result.score_percent}%</h1><p>{result.correct_count} of {result.question_count} correct. {result.mistakes_created ? "Your learning plan has been updated for revision." : "Great work—your mastery has been updated."}</p></section>{result.reviews.map((review, index) => <section className="recommendation" key={index}><p className="eyebrow">{review.is_correct ? "Correct" : "Review this"}</p><h2>{review.prompt}</h2><p>Your answer: {review.submitted_answer}</p>{!review.is_correct && <p>Correct answer: {review.correct_answer}</p>}<p>{review.explanation}</p></section>)}<Link className="button" href="/">Return to dashboard</Link></main>;
