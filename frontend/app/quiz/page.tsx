...
function QuizPageContent() {
  ...
  if (result) return <main className="form-page"><section className="quiz-celebration"><div className="celebration-dots" aria-hidden="true"><i /><i /><i /><i /><i /></div><p className="eyebrow">Practice complete</p><h1>{result.score_percent}%</h1><p className="xp-reward">+{5 + result.correct_count * 10} XP earned</p><p>{result.correct_count} of {result.question_count} correct. {result.mistakes_created ? "Your learning plan has been updated for revision." : "Great work—your mastery has been updated."}</p></section>{result.reviews.map((review, index) => <section className="recommendation" key={index}><p className="eyebrow">{review.is_correct ? "Correct" : "Review this"}</p><h2>{review.prompt}</h2><p>Your answer: {review.submitted_answer}</p>{!review.is_correct && <p>Correct answer: {review.correct_answer}</p>}<p>{review.explanation}</p></section>)}<Link className="button" href="/">Return to dashboard</Link></main>;
  if (!quiz) return <main className="form-page"><p>Preparing your quiz…</p></main>;
  return <main className="form-page"><p className="eyebrow">Quick challenge</p><h1>{quiz.title}</h1>{quiz.questions.map((question, index) => <section className="diagnostic-question" key={question.id}><p className="question-count">Question {index + 1} of {quiz.questions.length}</p><strong>{question.prompt}</strong>{question.options ? question.options.map((option) => <label key={option.label}><input type="radio" name={question.id} value={option.label} onChange={() => { setAnswers({ ...answers, [question.id]: option.label }); setAnsweredAt({ ...answeredAt, [question.id]: Date.now() }); setError(""); }} /> {option.label}. {option.text}</label>) : <input aria-label={`Answer for question ${index + 1}`} inputMode={question.question_type === "nat" ? "decimal" : "text"} placeholder="Your answer" value={answers[question.id] ?? ""} onChange={(event) => { setAnswers({ ...answers, [question.id]: event.target.value }); setAnsweredAt({ ...answeredAt, [question.id]: Date.now() }); setError(""); }} />}</section>)}<button onClick={submit}>Submit answer</button>{error && <p className="error">{error}</p>}</main>;
}
